...
The GCS plugin allows Fuel to deploy OpenStack with support of OpenStack Cinder
backup driver for Google Cloud Storage.
...
currently doesn’t support the backup driver installation and configuration.
...
Driver enablement and configuration will be done by Puppet manifests included
in plugin.
...
Limitations
-----------
Cinder does not support multiple backup backends at the same time so switchingi
...
* Create Fuel plugin bundle which allows setting plugin parameters and pass
  them to Openstack nodes via Hiera
