...
currently does not support Cinder Google Cloud Storage backup driver
installation and configuration out of the box.
...
Fuel GCS plugin should support environemnts with Cinder LVM backend and CEPH.
...
JSON format, and will be copied automatically to cinder nodes during the
deployment.
...
* Acceptance testing (this cases will be executed along with CI tests):

  * Block storage operations verified using Tempest framework with specific
    test cases:
...
* CI test cases:
  ...
  * System tests including deployment with different options enabled and
    plugin installation included, both LVM and Ceph options have to be
    verified as a Cinder back-end for all this cases:

    - Install plugin and deploy environment in HA/non-HA mode (one compute and
      one cinder node)
    - Install plugin and deploy environment in HA/non-HA mode with
      controller/cinder role assigned to a node
    - Install plugin and deploy environment in HA/non-HA mode with
      compute/cinder role assigned to a node
    - Remove, add controller node in HA mode cluster with plugin
    - Remove, add compute node in HA/non-HA mode cluster with plugin
    - Remove, add cinder node in HA/non-HA mode cluster with plugin
    - Remove, add controller/cinder node in HA mode cluster with plugin
    - Remove, add compute/cinder node in HA/non-HA mode cluster with plugin

  * Functional tests to verify plugin functionality are working correctly:

    - Backup Volume and restore it
    - Write/Read data to/from volume after backup
