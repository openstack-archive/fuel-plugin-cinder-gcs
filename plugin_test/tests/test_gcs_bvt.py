...
class GcsTestClass(GcsTestBase):
    ...
    @test(depends_on=[SetupEnvironment.prepare_slaves_3],
          groups=["deploy_gcs_non_ha_bvt", 'gcs_bvt'])
    @log_snapshot_after_test
    def deploy_gcs_non_ha_bvt(self):
        ...
        self.env.make_snapshot("deploy_gcs_non_ha_bvt")

    @test(depends_on=[SetupEnvironment.prepare_slaves_5],
          groups=["deploy_gcs_ha_bvt", 'gcs_bvt'])
    @log_snapshot_after_test
    def deploy_gcs_ha_bvt(self):
        ...
        self.fuel_web.run_ostf(
            ...
            test_sets=['smoke', 'sanity', 'ha', 'tests_platform',
                       'cloudvalidation'])
        self.env.make_snapshot("deploy_gcs_ha_bvt")
