...
class GcsTestBase(TestBasic):
    ...
    def set_attributes(self, cluster_id):
        ...
        attr['editable']['fuel-plugin-cinder-gcs']['metadata']['enabled'] = \
            True
        attr_short = attr['editable']['fuel-plugin-cinder-gcs']['metadata']
        attr_short['versions'][0]['backup_gcs_bucket_location']['value'] = \
            os.environ['GCS_LOCATION']
        attr_short['versions'][0]['backup_gcs_bucket']['value'] = \
            os.environ['GCS_BUCKET_NAME']
        attr_short['versions'][0]['backup_gcs_project_id']['value'] = \
    ...
    def upload_creds(remote):
        ...
        remote.upload(file_name, '/root/gcs/')

    @staticmethod
    def upload_rpm(remote):
        """Method designed to upload plugin rpm file."""
        logger.info('Uploading plugin RPM file to fuel master node.')
        file_name = os.environ['GCS_PLUGIN_RPM']
        remote.upload(file_name, '/root/')

    def install_plugin(self):
        """Method designed to install plugin on cluster."""
        master_remote = self.get_remote('master')
        self.upload_rpm(master_remote)
    ...
    def get_names(self, online_nodes):
        """Method designed to obtain fqdn nodes names list."""
        node_list = []
        for n in online_nodes:
            data = self.fuel_web.fqdn(n)
    ...
    def _verify_pacemaker(self, ctrl_node, online_nodes):
        ...
        online = self.get_names(online_nodes)
